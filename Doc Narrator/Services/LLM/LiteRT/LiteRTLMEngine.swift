...
public final class LiteRTLMEngine: @unchecked Sendable {
    ...
    ///   - maxNumTokens: Engine context budget (`litert_lm_engine_settings_set_max_num_tokens`).
    ///     Gemma 4 E4B has failed `engine_create` at 2048 in practice; 4096 matches the upstream Swift wrapper.
    public init(modelPath: URL, backend: String = "cpu", maxNumTokens: Int = 2048) {
        ...
        do {
            let createdEngine = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<OpaquePointer, any Error>) in
                self.inferenceQueue.async {
                    do {
                        // Keep logs at INFO while we diagnose E4B loads.
                        litert_lm_set_min_log_level(0)

                        // Safety: clear any per-model XNNPACK caches sitting next to the model file.
                        // These can become stale across app versions / setting changes and have been
                        // observed to cause an abort in MMapWeightCacheProvider during engine_create.
                        do {
                            let fm = FileManager.default
                            let parent = self.modelPath.deletingLastPathComponent()
                            let prefix = self.modelPath.lastPathComponent
                            if let entries = try? fm.contentsOfDirectory(at: parent, includingPropertiesForKeys: nil) {
                                for url in entries {
                                    let name = url.lastPathComponent
                                    guard name.hasPrefix(prefix), name.hasSuffix(".xnnpack_cache") else { continue }
                                    try? fm.removeItem(at: url)
                                }
                            }
                        }

                        // Text uses requested backend; vision/audio always run on CPU.
                        ...
                        // Context budget has a major impact on peak memory (KV cache + activations).
                        // For larger Gemma variants (E4B), forcing 4096 has been observed to make
                        // `litert_lm_engine_create` return NULL on-device.
                        let contextTokens = max(512, self.maxNumTokens)
                        litert_lm_engine_settings_set_max_num_tokens(settings, Int32(contextTokens))

                        // Avoid forcing activation dtype / prefill chunk size here.
                        // For Gemma 4 E4B, forcing non-default settings has been observed to make
                        // `litert_lm_engine_create` return NULL on-device.

                        // Large models: parallel section loading can sporadically fail engine creation.
                        litert_lm_engine_settings_set_parallel_file_section_loading(settings, false)

                        // Engine cache directory. IMPORTANT: this C API does NOT tolerate a NULL
                        // cache_dir pointer (it will crash trying to strlen(NULL)).
                        // We use a per-(model,backend) subdirectory so changes to backend config
                        // (e.g. enabling vision/audio CPU backends) trigger a fresh cache build.
                        let cacheRoot = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first!
                            .appendingPathComponent("litertlm_cache_v2", isDirectory: true)
                        let cacheURL = cacheRoot.appendingPathComponent(
                            "\(self.modelPath.lastPathComponent)_\(backendStr)",
                            isDirectory: true
                        )
                        try? FileManager.default.createDirectory(at: cacheURL, withIntermediateDirectories: true)
                        litert_lm_engine_settings_set_cache_dir(settings, cacheURL.path)
    ...
    public func audio(
        ...
        // Match Google AI Edge Gallery's Content.AudioBytes path: pass audio
        // as a base64 blob instead of a temp file path.
        let messageJSON = Self.buildMultimodalMessageJSON(
            audioBlobs: [audioData], imagePaths: [], text: prompt
        )
        return try await runConversationInference(
            messageJSON: messageJSON,
            tempURLs: [],
    ...
    // MARK: - Benchmark Stats (exposed for UI)

    /// Stats from the most recent session inference. Set on the main queue after each stream.
    ...
    /// Updated on main thread after every `sessionGenerateStreaming` call completes.
    @MainActor
    public private(set) var lastSessionStats: SessionStats?
    ...
    /// Ensure a persistent text session is open (opens one if missing).
    public func ensureSessionOpen(temperature: Float = 0.3, maxTokens: Int = 512) async throws {
        try ensureReady()
        if hasOpenSession { return }
        try await openSession(temperature: temperature, maxTokens: maxTokens)
    }
        ...
        try await withCheckedThrowingContinuation { (cont: CheckedContinuation<Void, Error>) in
            inferenceQueue.async { [self] in
                do {
                    ...
                    // LiteRT-LM only supports one active Session/Conversation at a time.
                    // Close the text chat session before opening a multimodal conversation.
                    if let s = chatSession {
                        logSessionBenchmark(s)
                        litert_lm_session_delete(s)
                        chatSession = nil
    ...
    public func conversationSend(
        ...
        // Prepare media files. Audio follows Google AI Edge Gallery's
        // Content.AudioBytes path (base64 blob), which preserves context
        // in the persistent Conversation API.
        var tempURLs: [URL] = []
        var audioBlobs: [Data] = []
        var imagePaths: [String] = []
        ...
        let messageJSON = Self.buildMultimodalMessageJSON(
            audioBlobs: audioBlobs, imagePaths: imagePaths, text: prompt
        )
        ...
        AsyncThrowingStream { continuation in
            self.inferenceQueue.async { [self] in
                ...
                do {
                    ...
                    guard let owned = strdup(messageJSON) else {
                        Unmanaged<StreamCallbackState>.fromOpaque(statePtr).release()
                        throw LiteRTLMError.inferenceFailure("Failed to allocate conversation message")
                    }
                    state.ownedInput = owned
                    ...
                    let result = litert_lm_conversation_send_message_stream(
                        ...
                        { callbackData, chunk, isFinal, errorMsg in
                            ...
                            if let chunk, errorMessage == nil {
                                ...
                                let extracted = LiteRTLMEngine.extractTextFromConversationResponse(text)
                                if !extracted.isEmpty { st.continuation.yield(extracted) }
                            }

                            if isFinal || errorMessage != nil {
                                if let raw = st.ownedInput {
                                    free(raw)
                                    st.ownedInput = nil
                                }
                                if let errorMessage {
                                    st.continuation.finish(throwing: LiteRTLMError.inferenceFailure(errorMessage))
                                } else {
                    ...
                    if result != 0 {
                        free(owned)
                        state.ownedInput = nil
                        Unmanaged<StreamCallbackState>.fromOpaque(statePtr).release()
                        throw LiteRTLMError.inferenceFailure("Failed to start conversation stream")
                    }
    ...
    public func sessionGenerateStreaming(input: String) -> AsyncThrowingStream<String, Error> {
        AsyncThrowingStream { continuation in
            self.inferenceQueue.async { [self] in
                ...
                // IMPORTANT: `litert_lm_session_generate_content_stream` is asynchronous and may
                // read the input buffer after this function returns. Do NOT pass a temporary
                // `withCString` pointer (use-after-free). Keep a heap copy alive until final.
                guard let owned = strdup(input) else {
                    Unmanaged<StreamCallbackState>.fromOpaque(statePtr).release()
                    continuation.finish(throwing: LiteRTLMError.inferenceFailure("Failed to allocate input buffer"))
                    return
                }

                var inputData = InputData(
                    type: kInputText,
                    data: UnsafeRawPointer(owned),
                    size: strlen(owned)
                )
                let result = litert_lm_session_generate_content_stream(
                    ...
                    { callbackData, chunk, isFinal, errorMsg in
                            ...
                            if isFinal || errorMessage != nil {
                                // Release the input buffer we strdup'd above.
                                if let raw = st.ownedInput {
                                    free(raw)
                                    st.ownedInput = nil
                                }
        ...
        AsyncThrowingStream { continuation in
            self.inferenceQueue.async { [self] in
                do {
                    ...
                    guard let owned = strdup(prompt) else {
                        Unmanaged<StreamCallbackState>.fromOpaque(statePtr).release()
                        litert_lm_session_delete(session)
                        litert_lm_session_config_delete(sessionConfig)
                        continuation.finish(throwing: LiteRTLMError.inferenceFailure("Failed to allocate input buffer"))
    ...
    private func logSessionBenchmark(_ session: OpaquePointer) {
        ...
        let stats = SessionStats(
            ...
            prefillTPS: lastPrefillTPS,
            decodeTPS: lastDecodeTPS
        )
        DispatchQueue.main.async { [weak self] in
            self?.lastSessionStats = stats
        }
    }
        ...
        return try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<String, any Error>) in
            self.inferenceQueue.async { [self, urlsToCleanup] in
                ...
                do {
                    ...
                    // The C++ runtime supports only one active Session/Conversation.
                    // If text chat pre-opened a Session, Conversation creation returns NULL.
                    if let s = chatSession {
                        logSessionBenchmark(s)
                        litert_lm_session_delete(s)
                        chatSession = nil
    ...
    nonisolated static func buildMultimodalMessageJSON(
        ...
        for blob in audioBlobs {
            contentItems.append(["type": "audio", "blob": blob.base64EncodedString()])
        }
        ...
        if !trimmedText.isEmpty {
            contentItems.append(["type": "text", "text": trimmedText])
        }
...
private final class StreamCallbackState: @unchecked Sendable {
    ...
    var ownedInput: UnsafeMutablePointer<CChar>?
